...
areacodes.each do |areacode| 
url_base = "https://api.postcodes.io/postcodes/#{areacode}/autocomplete?limit=10"
...
request = HTTParty.post("https://api.postcodes.io/postcodes", :body => { 'postcodes' => postcodes }.to_json, :headers => { 'Content-Type' => 'application/json' })

# byebug

request['result'].each{ |search| 
    if search === nil
        next
    else
        Postcode.create({ postcode: search['query'], borough_id: Borough.find_by(name: search['result']['admin_district']).id }) 
    end
}
